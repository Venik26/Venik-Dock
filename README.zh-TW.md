# Venik Dock for Rainmeter

[English](README.md) | **繁體中文**

一個簡單自訂、免翻設定檔、流暢的純原生 `.ini` Rainmeter 應用程式Dock。
 
---

## 核心特色

* **點擊即可自動使用PowerShell提取高解析度圖標，免去手動找去背圖的痛苦。**  
  
* **支援滾輪即時微調背景透明度，並可自由更換背景圖片。**   

* **純公式計算，右鍵一鍵切換多欄位排版，外觀自適應縮放。**  

* **完全採用 Rainmeter 原生公式，使用者不需要翻設定檔設定。**  

---

## 更新日誌  v1.0.3

<img width="1087" height="739" alt="Venik26 Rainmeter Dock User Interface Preview v1.0.3" src="https://github.com/user-attachments/assets/69d83ca6-5297-485c-8d9b-d9292f9891d0" />  

* **再次優化了抓圖的算法，win10的影片和音樂改為高解析度圖示**  

* **新增了滑鼠懸停的文件名稱顯示功能**  

* **新增支援`.lnk`連結、PWA程式**  

---

##  效果與功能預覽

### 中鍵設定模式
<img width="1062" height="763" alt="中鍵設定模式" src="https://github.com/user-attachments/assets/c2a2f2a6-5514-499e-a5a1-45e0119e6b3e" />

### 右鍵設定圖片與欄位
<img width="1062" height="763" alt="右鍵設定圖片" src="https://github.com/user-attachments/assets/53949dda-1ed9-4176-86af-306dd4099fc5" />

### 左鍵操作功能與刪除
<img width="1062" height="763" alt="左鍵操作功能" src="https://github.com/user-attachments/assets/5a589e7d-b60a-4edd-b85d-0d1593b8c410" />

---

## 系統需求

* **Rainmeter** (v4.5+) on **Windows 10 / 11**

---

## 安裝說明

1. 前往 **[Releases](https://github.com/Venik26/Venik-Dock/releases)** 下載頁面 
   
2. 下載最新的 `Venik Dock.rmskin` 安裝包 
   
3. 點兩下檔案，由 Rainmeter 自動完成安裝  


---

## 開源授權 (License)

本專案採用 [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh-hant) 創用 CC 授權條款釋出。
